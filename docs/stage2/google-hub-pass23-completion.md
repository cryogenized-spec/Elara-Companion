...
- Skipping a cooling-down intermediate tier is an explicit `skipUnhealthyFallbackModels` policy decision rather than an implicit routing side effect.
...
7. A later tier may be selected only when an intermediate tier is unavailable/cooling down and `skipUnhealthyFallbackModels` explicitly permits skipping it.
...
The tests prove the classification matrix, explicit unknown-error policy, deterministic preference-order fallback, retry/fallback separation, explicit unhealthy-tier skipping, and cooldown/recovery behaviour.
