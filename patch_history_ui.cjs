...
// Update state definition if not already updated completely
code = code.replace(
...
// We also need to fix occurrences of setHistoryModal({isOpen: true, selectedRevisionId: null, isComparing: false})
// to include compareRevisionId: null
code = code.replace(
  `onClick={() => setHistoryModal({isOpen: true, selectedRevisionId: null, isComparing: false})}`,
  `onClick={() => setHistoryModal({isOpen: true, selectedRevisionId: null, isComparing: false, compareRevisionId: null})}`
);
