...
const makeMemory = (index: number, mode: 'unique' | 'shared'): MemoryItem => ({
  id: `bench-${mode}-${index}`,
  content: mode === 'shared'
    ? `User is working on the same active project item ${index % 200}.`
    : `User note topic${index} detail${index} context${index}.`,
  kind: 'observation',
...
for (const size of sizes) {
  for (const mode of ['unique', 'shared'] as const) {
