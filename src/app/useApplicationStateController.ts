...
export function useApplicationStateController(): ApplicationState {
  const [isLoaded, setIsLoaded] = useState(false); const [conversations, setConversations] = useState<Conversation[]>([]); const [folders, setFolders] = useState<Folder[]>([]); const [activeId, setActiveId] = useState<string | null>(null); const [settings, setSettings] = useState<ElaraSettings>(DEFAULT_SETTINGS); const [worldState, setWorldState] = useState<WorldState>(DEFAULT_WORLD_STATE); const [memoryState, setMemoryState] = useState<MemoryScratchpadState>(DEFAULT_MEMORY_STATE); const [customPortrait, setCustomPortrait] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const googleSettings = settings as GooglePersistedSettings;
    if (Array.isArray(googleSettings.googleGrantedScopes) && googleSettings.googleGrantedScopes.length > 0) return;
    googleRestoreAttempted.current = true;
    void requestGoogleBaseAuthorization(false).catch(() => undefined);
  }, [isLoaded, settings.googleStayConnected, settings]);
