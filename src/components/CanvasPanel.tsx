...
import { createGoogleDoc, createKeepNote, isGoogleConnected } from '../lib/googleApi';
...
export const CanvasPanel: React.FC<CanvasPanelProps> = ({ canvas, onClose, onUpdateContent }) => {
  ...
  return (
    <div className="w-80 md:w-96 lg:w-[520px] border-l border-zinc-800 bg-[#0c0c0e] flex flex-col shrink-0 z-20 transition-all duration-300 shadow-2xl">
      ...
      <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/80 shrink-0">
        ...
        <div className="flex items-center gap-1 shrink-0 ml-2">
          ...
          <button
            ...
            title={viewMode === 'edit' ? 'Switch to Markdown Preview' : 'Switch to Editor'}
          >
          ...
          <button
            ...
            disabled={savingKeep}
            className="flex items-center gap-1 px-2 py-1.5 rounded-lg bg-amber-950/50 hover:bg-amber-900/60 text-amber-300 border border-amber-700/40 text-[11px] font-medium transition-colors disabled:opacity-50"
            title="Save to Google Keep Archive"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1.5 rounded-lg bg-emerald-950/50 hover:bg-emerald-900/60 text-emerald-300 border border-emerald-700/40 text-[11px] font-medium transition-colors disabled:opacity-50"
          ...
          <button
            onClick={handleCopy}
            className="p-1.5 rounded-lg bg-zinc-800/80 hover:bg-zinc-700 text-zinc-200 text-xs transition-colors border border-zinc-700/50"
      ...
      {statusMessage && (
        <div
          className={`px-3 py-2 text-xs flex items-center justify-between gap-2 shrink-0 border-b ${
        ...
        >
          ...
          {statusMessage.link && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1 underline text-emerald-400 hover:text-emerald-200 shrink-0 font-medium text-[11px]"
            >
              <span>Open</span>
      ...
      <div className="flex-1 bg-zinc-950 overflow-hidden flex flex-col relative">
        {viewMode === 'edit' ? (
          <textarea
            ...
            className="w-full h-full p-4 bg-transparent text-xs font-mono text-zinc-200 resize-none outline-none custom-scrollbar leading-relaxed selection:bg-sky-500/30"
            spellCheck="false"
          />
        ) : (
          <div className="w-full h-full p-4 overflow-y-auto custom-scrollbar text-xs text-zinc-200 leading-relaxed space-y-3 font-sans select-text">
