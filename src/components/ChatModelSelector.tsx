...
import { createModelPreferenceState, getModelPreferenceOptions, type ModelPreferenceState } from '../lib/modelPreference';
...
export const ChatModelSelector: React.FC = () => {
  ...
  const selectModel = async (modelId: string) => {
    ...
    const nextReliability = normalizeReliabilitySettings({
    ...
    const nextSettings: ElaraSettings = {
      ...settings,
      model: modelId,
      reliabilitySettings: nextReliability,
    };
    ...
    try {
      ...
      // Persistence failure leaves the current selection visible and the menu open.
    }
  };

  const selectedLabel = settings?.model
    ? options.find((option) => option.id === settings.model)?.name || settings.model
    : 'Model';
