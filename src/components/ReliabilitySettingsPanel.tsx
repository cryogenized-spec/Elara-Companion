  ...
  return (
    <div className={embedded ? 'flex min-h-0 flex-1 flex-col overflow-hidden' : 'fixed right-3 top-3 bottom-3 z-[90] flex w-[min(430px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-2xl border border-zinc-700 bg-[#111113] shadow-2xl shadow-black/50'}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        ...
        <Toggle checked={draft.autoRestorePreferredModel} onChange={(value) => update({ autoRestorePreferredModel: value })} label="Automatically restore preferred model" description="After its cooldown, probe the preferred model and return to it when healthy." />
        <Toggle checked={draft.honorRetryAfter} onChange={(value) => update({ honorRetryAfter: value })} label="Honor server retry timing" description="Respect Gemini's Retry-After guidance when it is supplied." />

        <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3.5 space-y-3"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold text-zinc-200">Retry attempts</p><p className="text-[11px] text-zinc-500">Maximum total attempts on the same model.</p></div><span className="font-mono text-xs text-sky-400">{draft.maxAttempts}</span></div><input type="range" min="1" max="5" step="1" value={draft.maxAttempts} onChange={(e) => update({ maxAttempts: Number(e.target.value) })} className="w-full accent-sky-500" /><div className="flex justify-between text-[10px] text-zinc-600"><span>1</span><span>3 (Default)</span><span>5</span></div></div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3.5"><div className="flex items-center gap-2"><Zap className="h-3.5 w-3.5 text-amber-400" /><p className="text-xs font-semibold text-zinc-200">Fallback model order</p></div><p className="mt-1 text-[11px] text-zinc-500">Only enabled models can be used; first healthy entry wins.</p><div className="mt-3 space-y-2">{RELIABILITY_FALLBACK_MODELS.map((model) => { const enabled = selectedFallbacks.has(model); const index = draft.fallbackModels.indexOf(model); return <div key={model} className={`flex items-center gap-2 rounded-lg border px-2.5 py-2 ${enabled ? 'border-zinc-700 bg-zinc-900' : 'border-zinc-900 bg-zinc-950/40 opacity-60'}`}><input type="checkbox" checked={enabled} onChange={() => toggleFallback(model)} className="accent-sky-500" /><span className="flex-1 text-xs text-zinc-200">{MODEL_LABELS[model]}</span>{enabled && <span className="font-mono text-[10px] text-zinc-500">#{index + 1}</span>}<button type="button" disabled={!enabled || index <= 0} onClick={() => moveFallback(index, -1)} className="rounded p-1 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200 disabled:opacity-25"><ArrowUp className="h-3.5 w-3.5" /></button><button type="button" disabled={!enabled || index === draft.fallbackModels.length - 1} onClick={() => moveFallback(index, 1)} className="rounded p-1 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200 disabled:opacity-25"><ArrowDown className="h-3.5 w-3.5" /></button></div>; })}</div></div>

        <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3.5 space-y-3"><div><p className="text-xs font-semibold text-zinc-200">Retry / failover conditions</p><p className="mt-1 text-[11px] text-zinc-500">Choose which classified failures are allowed to trigger recovery behavior.</p></div>{ERROR_OPTIONS.map((item) => { const key = item.group === 'retry' ? 'retryableErrorCodes' : 'failoverErrorCodes'; const checked = draft[key].includes(item.code); return <label key={`${item.group}-${item.code}`} className="flex items-start gap-2.5 rounded-lg border border-zinc-900 bg-zinc-950/50 p-2.5 cursor-pointer"><input type="checkbox" checked={checked} onChange={() => toggleErrorCode(key, item.code)} className="mt-0.5 accent-sky-500" /><span className="min-w-0"><span className="block text-[11px] font-medium text-zinc-200">{item.label}</span><span className="block text-[10px] leading-relaxed text-zinc-600">{item.description}</span></span></label>; })}</div>
