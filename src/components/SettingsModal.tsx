...
const tabs: readonly { id: SettingsTab; label: string; description: string }[] = [
  { id: 'persona', label: 'Persona', description: 'Identity and response behavior' },
  { id: 'visuals', label: 'Visuals', description: 'Appearance and display' },
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSaveSettings, customPortrait, onUploadPortrait, onRemovePortrait, onExportAllData, onImportData, onClearAllData }) => {
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 p-2 backdrop-blur-md sm:p-4" role="dialog" aria-modal="true" aria-label="Settings">
    <div className="flex h-[96vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0b0b0b] text-white shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[220px_minmax(0,1fr)]">
        <nav className="border-b border-white/10 p-3 md:border-b-0 md:border-r" aria-label="Settings sections"><div className="space-y-1">{tabs.map(tab => <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`w-full rounded-xl border px-3 py-3 text-left transition ${activeTab === tab.id ? 'border-white/15 bg-white/[0.08]' : 'border-transparent hover:border-white/10 hover:bg-white/[0.04]'}`}><span className="block text-sm font-medium">{tab.label}</span><span className="mt-0.5 block text-[11px] leading-4 text-white/40">{tab.description}</span></button>)}</div><div className="mt-4 hidden rounded-xl border border-white/10 bg-white/[0.025] p-3 text-xs text-white/40 md:block">Google services live in Google Hub.</div></nav>
        <main className="min-h-0 overflow-y-auto p-5 sm:p-6">
          {activeTab === 'persona' && <section className="space-y-5"><SectionTitle title="Persona" description="Identity, model choice, and saved persona snapshots."/><Field label="Your name"><input value={formData.userName} onChange={e => update({ userName: e.target.value })} className={inputClass}/></Field><Field label="Model"><select value={formData.model} onChange={e => update({ model: e.target.value })} className={inputClass}>{modelOptions.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}</select></Field><Field label="Temperature"><input type="number" min="0" max="2" step="0.05" value={formData.temperature} onChange={e => update({ temperature: Number(e.target.value) })} className={inputClass}/></Field><button type="button" onClick={() => setShowSnapshotPrompt(true)} className={buttonClass}>Save persona snapshot</button>{showSnapshotPrompt && <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><input autoFocus value={snapshotName} onChange={e => setSnapshotName(e.target.value)} placeholder="Snapshot name" className={inputClass}/><div className="mt-3 flex gap-2"><button type="button" onClick={() => void handleSaveSnapshot()} className={buttonClass}>Save</button><button type="button" onClick={() => setShowSnapshotPrompt(false)} className={buttonClass}>Cancel</button></div></div>}{snapshots.length > 0 && <div className="space-y-2"><p className="text-sm font-medium">Saved snapshots</p>{snapshots.map(snapshot => <div key={snapshot.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.02] p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm">{snapshot.name}</p><p className="text-xs text-white/35">{new Date(snapshot.timestamp).toLocaleString()}</p></div><button type="button" onClick={() => loadSnapshot(snapshot)} className={buttonClass}>Load</button><button type="button" onClick={() => void deleteSnapshot(snapshot.id)} className={buttonClass}>Delete</button></div>)}</div>}</section>}
          {activeTab === 'visuals' && <section className="space-y-5"><SectionTitle title="Visuals" description="Appearance and display controls."/><div className="grid gap-5 lg:grid-cols-2"><div className="space-y-4"><Field label="Portrait scale"><input type="range" min="0.7" max="1.5" step="0.05" value={formData.portraitScale} onChange={e => update({ portraitScale: Number(e.target.value) })} className="w-full"/></Field><Field label="Theme"><select value={formData.themeMode || formData.theme} onChange={e => update({ themeMode: e.target.value as ElaraSettings['themeMode'], theme: e.target.value === 'light' ? 'light' : 'dark' })} className={inputClass}><option value="system">System</option><option value="dark">Dark</option><option value="light">Light</option></select></Field><Field label="Timezone"><input value={formData.timezone} onChange={e => update({ timezone: e.target.value })} className={inputClass}/></Field></div><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><img src={activePortraitImage} alt="Elara portrait" className="mx-auto h-40 w-40 rounded-2xl object-cover" style={{ transform: `scale(${formData.portraitScale})` }}/><div className="mt-5 flex flex-wrap justify-center gap-2"><input ref={portraitInputRef} type="file" accept="image/*" className="hidden" onChange={e => void handlePortraitFile(e.target.files?.[0])}/><button type="button" onClick={() => portraitInputRef.current?.click()} className={buttonClass}>Upload portrait</button><button type="button" onClick={onRemovePortrait} className={buttonClass}>Reset portrait</button></div></div></div><div className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4"><Field label="Backdrop opacity"><input type="range" min="0" max="1" step="0.05" value={formData.backdropOpacity} onChange={e => update({ backdropOpacity: Number(e.target.value) })} className="w-full"/></Field><Field label="Backdrop blur"><input type="range" min="0" max="30" step="1" value={formData.backdropBlur} onChange={e => update({ backdropBlur: Number(e.target.value) })} className="w-full"/></Field><input ref={backdropInputRef} type="file" accept="image/*" className="hidden" onChange={e => void handleBackdropFile(e.target.files?.[0])}/><button type="button" onClick={() => backdropInputRef.current?.click()} className={buttonClass}>Choose backdrop</button>{formData.backdropImage && <button type="button" onClick={() => update({ backdropImage: null })} className={buttonClass}>Remove backdrop</button>}</div></section>}
