...
export const ThinkingTimeline: React.FC<ThinkingTimelineProps> = ({
  ...
  const duration = typeof thoughtDurationMs === 'number'
      ...
      <div className="flex items-center gap-2 px-1 pb-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[10px] text-zinc-500">
            {thoughts.length} {thoughts.length === 1 ? 'step' : 'steps'}{duration ? ` · ${duration}` : ''}
          </div>
        </div>
      </div>
      ...
      <div className="relative pl-7">
        ...
        <div className="space-y-1">
          {thoughts.map((step, index) => {
            ...
            return (
              <div key={step.id || `${step.timestamp}-${index}`} className="relative">
                <div className="absolute -left-7 top-2.5 z-10 flex h-5 w-5 items-center justify-center rounded-full border border-pink-400/60 bg-zinc-950 text-[9px] font-semibold text-pink-300">
                  {index + 1}
                </div>

                <button
                  type="button"
                  onClick={() => toggle(step.id || String(index))}
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className={`block text-xs font-medium ${isLast && isStreaming ? 'text-pink-200' : 'text-zinc-200'}`}>
                      {step.step_title}
                    </span>
                    {isOpen && (
