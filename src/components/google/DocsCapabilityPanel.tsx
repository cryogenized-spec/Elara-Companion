...
export function DocsCapabilityPanel({ canUse, onActivity }: DocsCapabilityPanelProps) {
  ...
  const create = async () => { if (!canUse || !newTitle.trim()) return; setError(null); try { const result = await createGoogleDoc(newTitle.trim(), newContent); setNotice(`Created “${result.title}”.`); setNewTitle(''); setNewContent(''); onActivity?.(`Created Google Doc “${result.title}”`, true); setSelected({ ...result, content: newContent.trim() }); } catch (err) { setError(err instanceof Error ? err.message : 'Unable to create document.'); } };
