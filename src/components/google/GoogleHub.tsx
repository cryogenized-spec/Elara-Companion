...
export function GoogleHub({ accountEmail, authorization, capabilities, activity = [], capabilityPanels = {}, onOpenCapability, onEnableCapability, onDisconnect, onOpenGoogleAccount }: GoogleHubProps) {
  ...
  const capabilityState = useMemo(() => new Map(capabilities.map((capability) => [capability.id, capability.requiredCapabilities.length > 0 && capability.requiredCapabilities.every((required) => authorization.grantedCapabilities.includes(required))])), [authorization.grantedCapabilities, capabilities]);
  ...
  return <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/20 text-white">
    <header className="border-b border-white/10 px-5 py-5 sm:px-6">
      ...
      {selectedCapability ? <button type="button" onClick={() => setSelectedCapabilityId(null)} className="mt-4 rounded-lg border border-white/10 px-3 py-2 text-xs font-medium text-white/70 hover:bg-white/[0.06]">← Back to Google services</button> : <nav className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4" aria-label="Google Hub sections">{SECTIONS.map((item) => <button key={item.id} type="button" onClick={() => setSection(item.id)} className={`rounded-xl border px-3 py-3 text-left transition ${section === item.id ? 'border-white/20 bg-white/10' : 'border-white/5 bg-white/[0.03] hover:border-white/10 hover:bg-white/[0.06]'}`}><span className="block text-sm font-medium">{item.label}</span><span className="mt-0.5 block text-[11px] leading-4 text-white/45">{item.description}</span></button>)}</nav>}
    </header>
    <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-6">
      {selectedCapability && selectedPanel}
      {!selectedCapability && section === 'account' && <AccountPanel accountEmail={accountEmail} authorization={authorization} enabledCount={enabledCount} onDisconnect={onDisconnect} onOpenGoogleAccount={onOpenGoogleAccount} />}
      ...
      {!selectedCapability && section === 'permissions' && <PermissionsPanel capabilities={capabilities} capabilityState={capabilityState} authorization={authorization} onOpenCapability={openCapability} onEnableCapability={onEnableCapability} />}
    </div>
  </section>;
}

function AccountPanel({ accountEmail, authorization, enabledCount, onDisconnect, onOpenGoogleAccount }: { accountEmail?: string; authorization: GoogleHubAuthorizationSnapshot; enabledCount: number; onDisconnect?: () => void; onOpenGoogleAccount?: () => void; }) {
  return <div className="mx-auto max-w-2xl space-y-5"><PanelTitle eyebrow="Account" title="Google account" description="Identity belongs here; individual service permissions belong elsewhere." /><div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/10 text-lg font-semibold">G</div><div className="min-w-0 flex-1"><p className="truncate text-base font-medium">{accountEmail || 'Google account'}</p><p className="mt-1 text-sm text-white/50">{authorization.status === 'authorized' ? 'Connected' : authorization.status === 'partially-authorized' ? 'Connected with limited capabilities' : 'Not connected'}</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><Stat label="Capabilities enabled" value={String(enabledCount)} /><Stat label="Authorization state" value={authorization.status} /><Stat label="Last updated" value={new Date(authorization.updatedAt).toLocaleString()} /><Stat label="Credential state" value="Provider-managed" /></div></div><div className="flex flex-wrap gap-2"><button type="button" onClick={onOpenGoogleAccount} className="rounded-xl border border-white/10 bg-white/[0.05] px-4 py-2.5 text-sm font-medium hover:bg-white/[0.08]">Manage Google account</button><button type="button" onClick={onDisconnect} disabled={!authorization.authorized || !onDisconnect} className="rounded-xl border border-red-400/20 bg-red-400/[0.06] px-4 py-2.5 text-sm font-medium text-red-200 disabled:cursor-not-allowed disabled:opacity-40">Disconnect Google</button></div></div>;
}
...
function PermissionsPanel({ capabilities, capabilityState, authorization, onOpenCapability, onEnableCapability }: { capabilities: readonly GoogleHubCapabilityDescriptor[]; capabilityState: Map<GoogleHubCapabilityDescriptor['id'], boolean>; authorization: GoogleHubAuthorizationSnapshot; onOpenCapability: (capability: GoogleHubCapabilityDescriptor) => void; onEnableCapability?: (capability: GoogleHubCapabilityDescriptor) => void; }) { return <div className="space-y-5"><PanelTitle eyebrow="Permissions" title="Google permissions" description="Capability access is explicit and independent; credentials never live in this UI state." /><div className="space-y-2">{capabilities.map((capability) => { const enabled = capabilityState.get(capability.id) === true; const missing = authorization.missingCapabilities.filter((item) => capability.requiredCapabilities.includes(item)); return <div key={capability.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3"><div className={`h-2.5 w-2.5 rounded-full ${enabled ? 'bg-emerald-300' : 'bg-white/20'}`} /><div className="min-w-0 flex-1"><p className="text-sm font-medium">{capability.name}</p><p className="mt-0.5 text-xs text-white/40">{enabled ? 'Access granted' : missing.length ? `${missing.length} permission${missing.length === 1 ? '' : 's'} missing` : 'Unavailable'}</p></div><button type="button" onClick={() => onOpenCapability(capability)} className="rounded-lg border border-white/10 px-3 py-2 text-xs font-medium hover:bg-white/[0.06]">Details</button>{!enabled && onEnableCapability && <button type="button" onClick={() => onEnableCapability(capability)} className="rounded-lg border border-amber-300/20 bg-amber-300/[0.06] px-3 py-2 text-xs font-medium text-amber-100 hover:bg-amber-300/[0.10]">Enable</button>}</div>; })}</div></div>; }
