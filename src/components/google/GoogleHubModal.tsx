...
export function GoogleHubModal({ isOpen, onClose, onAskElara }: GoogleHubModalProps) {
  ...
  const enableCapability = async (descriptor: GoogleHubCapabilityDescriptor) => {
    try {
      ...
      for (const capability of requested) if (!isGranted(capability)) await googleIdentity.requestCapabilityAuthorization(googleCapabilities.getScopes(capability), false);
      refresh();
