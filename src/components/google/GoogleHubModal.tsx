...
export function GoogleHubModal({ isOpen, onClose, onAskElara }: GoogleHubModalProps) {
  ...
  useEffect(() => {
    if (!isOpen || !authorization.authorized) { setAccountEmail(undefined); return; }
    let cancelled = false;
    googleIdentity.getAccountEmail().then((email) => { if (!cancelled) setAccountEmail(email || undefined); }).catch(() => { if (!cancelled) setAccountEmail(undefined); });
  ...
  const context = (descriptor: GoogleHubCapabilityDescriptor) => ({ descriptor, isGranted, recordActivity: (capabilityId: string, action: 'read' | 'create' | 'update' | 'delete' | 'send' | 'open', description: string, reversible = false) => {
    googleActivityRecorder.record({ id: `gha_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, timestamp: Date.now(), capabilityId, action, description, reversible, external: action !== 'open' });
    setActivityTick((value) => value + 1);
  }});
