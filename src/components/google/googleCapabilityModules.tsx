...
export interface GoogleCapabilityModuleRegistry {
  register(id: GoogleHubCapabilityId, factory: GoogleCapabilityModuleFactory): void;
  unregister(id: GoogleHubCapabilityId): void;
  has(id: GoogleHubCapabilityId): boolean;
  render(descriptor: GoogleHubCapabilityDescriptor, context: GoogleHubPanelContext): React.ReactNode;
}
...
class DefaultGoogleCapabilityModuleRegistry implements GoogleCapabilityModuleRegistry {
  ...
  register(id: GoogleHubCapabilityId, factory: GoogleCapabilityModuleFactory): void {
    if (!id.trim()) throw new Error('Google capability module id must not be empty');
    if (this.factories.has(id)) throw new Error(`Google capability module already registered: ${id}`);
...
export function createGoogleCapabilityModuleRegistry(): GoogleCapabilityModuleRegistry {
  return new DefaultGoogleCapabilityModuleRegistry();
}
...
googleCapabilityModuleRegistry.register('sheets', c => <SheetsCapabilityPanel canRead={c.isGranted('sheets.read')} canWrite={c.isGranted('sheets.write')} onActivity={(d,r)=>c.recordActivity('sheets','update',d,r)}/>);
...
export function createGoogleCapabilityModule(descriptor: GoogleHubCapabilityDescriptor): GoogleCapabilityModule {
  return { descriptor, renderPanel: context => googleCapabilityModuleRegistry.render(descriptor, context) };
}

export function createGoogleCapabilityModules(descriptors: readonly GoogleHubCapabilityDescriptor[]): readonly GoogleCapabilityModule[] {
