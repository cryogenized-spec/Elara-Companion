...
const makeMemory = (overrides: Partial<MemoryItem> = {}): MemoryItem => ({
  id: 'mem-1', content: 'Elara remembers the workshop plan.', kind: 'project', lifecycle: 'persistent', source: 'conversation',
  ...
  tags: [], reinforcementCount: 0, ...overrides,
});
...
describe('memory maintenance', () => {
  ...
  it('flags stale records according to lifecycle windows', () => {
    ...
    const plan = buildMemoryMaintenancePlan([
      makeMemory({ id: 'working', lifecycle: 'working', kind: 'working', updatedAt: '2026-08-01T00:00:00.000Z' }),
  ...
  it('protects pinned and core memories from expiry', () => {
    ...
    const plan = buildMemoryMaintenancePlan([
      ...
      makeMemory({ id: 'core', expiresAt: '2026-08-18T00:00:00.000Z', lifecycle: 'core', importance: 'core' }),
    ], { now });
