...
describe('thinking timeline UI contract', () => {
  ...
  it('keeps the user-facing summary separate from the expanded detail', () => {
    assert.equal(steps[1].step_title, 'Check context');
    assert.equal(steps[1].summary, 'Checking relevant context.');
  });

  it('does not depend on the removed modal presentation', async () => {
    const source = await import('node:fs/promises');
    const modalPath = new URL('../../components/ThoughtLogModal.tsx', import.meta.url);
    await assert.rejects(source.stat(modalPath));
  });
});
