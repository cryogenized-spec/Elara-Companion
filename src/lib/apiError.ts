...
export function classifyApiError(error: any, modelId?: string): ClassifiedApiError {
  ...
  if (lower.includes('safety') || lower.includes('blocked by safety') || lower.includes('content policy')) {
    return {
      ...
      modelId,
      message: 'The model stopped this response because of its content-safety controls. Nothing was lost; you can revise the request and try again.',
      retryable: false,
      rawMessage,
    };
  }
  ...
  if (status === 401 || lower.includes('api key') && (lower.includes('invalid') || lower.includes('unauthorized'))) {
    return {
      ...
      httpStatus: 401,
      modelId,
      message: 'Gemini rejected the API key. Check the key in Settings and try again.',
      retryable: false,
      rawMessage,
    };
  }
  ...
  if (status === 403 || lower.includes('permission denied') || lower.includes('forbidden')) {
    return {
      code: 'API_FORBIDDEN_403',
      httpStatus: 403,
      modelId,
      message: `Gemini denied access to ${modelId ? `[${modelId}]` : 'this model'}. Check project permissions, billing, or model availability.`,
      retryable: false,
      rawMessage,
    };
  }
  ...
  if (status === 404 || lower.includes('not found') || lower.includes('not_found') || lower.includes('model does not exist')) {
    return {
      ...
      httpStatus: 404,
      modelId,
      message: `The requested Gemini model ${modelId ? `[${modelId}] ` : ''}was not found or is not available to this project. Choose another active model in Settings.`,
      retryable: false,
      rawMessage,
    };
  }
  ...
  if (status === 503 || lower.includes('unavailable') || lower.includes('overloaded')) {
    return {
      ...
      httpStatus: 503,
      modelId,
      message: `Gemini is temporarily unavailable or overloaded for ${modelId ? `[${modelId}]` : 'this request'}. Your conversation is safe; retry shortly.`,
      retryable: true,
      retryAfterMs: retryAfterMs(error) || 5000,
      rawMessage,
    };
  }
  ...
  if (status === 400) {
    return {
      ...
      httpStatus: 400,
      modelId,
      message: `Gemini rejected the request as invalid${modelId ? ` for [${modelId}]` : ''}. Check the selected model and its generation settings.`,
      retryable: false,
      rawMessage,
    };
  }
  ...
  if (lower.includes('failed to fetch') || lower.includes('network') || lower.includes('connection')) {
    return {
      code: 'NETWORK_ERROR',
      modelId,
      message: 'The connection to Gemini failed. Your conversation is intact; check the network connection and retry.',
      retryable: true,
      retryAfterMs: 2000,
      rawMessage,
    };
  ...
  return {
    ...
    httpStatus: status,
    modelId,
    message: `Gemini returned an unexpected error${modelId ? ` for [${modelId}]` : ''}. Your conversation is safe; retry or select another model.`,
    retryable: true,
    retryAfterMs: 2000,
    rawMessage,
  };
