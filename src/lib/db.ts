...
export async function setDbConversations(data: Conversation[]) {
...
export async function getDbPortrait(): Promise<string | null> {
  return (await get(PORTRAIT_KEY)) || null;
}
export async function setDbPortrait(data: string | null) {
  if (data) await set(PORTRAIT_KEY, data);
  else await del(PORTRAIT_KEY);
}
...
export async function setDbFolders(data: Folder[]) {
  await set(FOLDERS_KEY, data);
}
...
export async function setDbSnapshots(data: PersonaSnapshot[]) {
  await set(SNAPSHOTS_KEY, data);
}
