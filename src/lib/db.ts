...
export async function migrateFromLocalStorage(): Promise<{ migrated: boolean; failures: string[] }> {
  ...
  for (const [idbKey, legacyKey, transform] of migrations) { const migrated = await migrateValue(idbKey, legacyKey, transform); if (!migrated && readLegacy(legacyKey)) failures.push(legacyKey); }
  if (failures.length === 0) { await set(MIGRATION_KEY, true); console.log('Migration complete.'); } else console.warn('Migration completed with recoverable failures:', failures);
