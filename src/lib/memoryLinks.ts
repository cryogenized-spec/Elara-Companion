...
export function normalizeMemoryLinks(memory: Pick<MemoryItem, 'links' | 'sourceConversationId' | 'sourceArtifactId'>): MemoryLink[] {
  ...
  if (memory.sourceConversationId) {
    links.set(`conversation:${memory.sourceConversationId}`, { type: 'conversation', id: memory.sourceConversationId });
  }

  if (memory.sourceArtifactId) {
    links.set(`artifact:${memory.sourceArtifactId}`, { type: 'artifact', id: memory.sourceArtifactId });
  }
...
export function buildMemoryLinks(context: MemoryLinkContext): MemoryLink[] {
  ...
  for (const artifactId of context.artifactIds || []) {
    links.push({ type: 'artifact', id: artifactId, label: context.artifactLabels?.[artifactId] });
  }
...
export function memoriesLinkedTo(memories: MemoryItem[], linkType: MemoryLink['type'], linkId: string): MemoryItem[] {
  if (!linkId) return [];
  return memories.filter((memory) => getMemoryLinks(memory, linkType).some((link) => link.id === linkId));
}
