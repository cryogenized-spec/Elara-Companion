...
test('Google Hub authorization reports partial capability coverage', () => {
  const state = createGoogleHubAuthorizationState(
    capabilities,
    () => true,
    (capability) => capability === 'gmail.read' || capability === 'calendar.read',
    () => 456,
  );
  ...
  assert.deepEqual(snapshot.grantedCapabilities, ['gmail.read', 'calendar.read']);
  assert.deepEqual(snapshot.missingCapabilities, ['gmail.compose', 'calendar.write']);
});
...
test('Google Hub authorization reports unauthorized when identity is not authorized', () => {
  ...
  assert.equal(snapshot.authorized, false);
