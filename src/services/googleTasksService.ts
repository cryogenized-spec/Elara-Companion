...
function normalizeTask(raw: any, list?: TaskListItem): TaskItem {
  return {
    ...
    ...(raw?.notes !== undefined ? { notes: String(raw.notes) } : {}),
    status: raw?.status === 'completed' ? 'completed' : 'needsAction',
    ...(raw?.due ? { due: String(raw.due) } : {}),
    ...(raw?.completed ? { completed: String(raw.completed) } : {}),
    ...(raw?.updated ? { updated: String(raw.updated) } : {}),
    ...(raw?.parent ? { parent: String(raw.parent) } : {}),
    ...(raw?.position ? { position: String(raw.position) } : {}),
    ...(raw?.deleted ? { deleted: true } : {}),
    ...(raw?.hidden ? { hidden: true } : {}),
    ...(Array.isArray(raw?.links) ? {
      links: raw.links.filter((link: any) => link?.link).map((link: any) => ({
        type: String(link.type || 'link'),
        ...(link.description ? { description: String(link.description) } : {}),
        link: String(link.link),
      })),
    } : {}),
    ...(raw?.webViewLink ? { webViewLink: String(raw.webViewLink) } : {}),
    ...(raw?.assignmentInfo ? { assignmentInfo: raw.assignmentInfo } : {}),
    ...(raw?.spaceInfo ? { spaceInfo: raw.spaceInfo } : {}),
    ...(list ? { listId: list.id, listTitle: list.title } : {}),
  };
}

async function fetchTaskListsPage(token: string, pageToken?: string): Promise<{ items: TaskListItem[]; nextPageToken?: string }> {
  const params = new URLSearchParams({ maxResults: String(MAX_PAGE_SIZE) });
  ...
  return {
    items: (data.items || []).map((list: any) => ({
      id: String(list.id),
      title: String(list.title || 'Tasks'),
      ...(list.updated ? { updated: String(list.updated) } : {}),
    })),
...
export async function getTaskLists(explicitToken?: string): Promise<{ items: TaskListItem[]; nextPageToken?: string }> {
  ...
  do {
    const page = await fetchTaskListsPage(token, pageToken);
...
async function fetchTaskPage(
  ...
  list: TaskListItem,
): Promise<{ items: TaskItem[]; nextPageToken?: string }> {
  const params = new URLSearchParams({
    ...
    showDeleted: String(options.showDeleted ?? false),
  });
  if (pageToken) params.set('pageToken', pageToken);
  ...
  const data = await res.json();
  return {
    items: (data.items || []).map((task: any) => normalizeTask(task, list)),
    nextPageToken: data.nextPageToken ? String(data.nextPageToken) : undefined,
  };
}
...
export async function listTasks(taskListId: string, explicitToken?: string, options: ListTasksOptions = {}): Promise<ListTasksResult> {
  ...
  const list = lists.items.find((item) => item.id === taskListId);
  ...
  do {
    const page = await fetchTaskPage(taskListId, token, pageToken, options, list);
    totalFetched += page.items.length;
    const remaining = maxTotalTasks - items.length;
    items.push(...page.items.slice(0, Math.max(0, remaining)));
    if (page.items.length > remaining) truncated = true;
    pageToken = page.nextPageToken;
    if (items.length >= maxTotalTasks) {
...
export async function listAllPendingTasks(explicitToken?: string, options: Omit<ListTasksOptions, 'showCompleted'> = {}): Promise<{ items: TaskItem[]; lists: TaskListItem[]; truncated: boolean; totalFetched: number }> {
  ...
  for (const list of lists.items) {
    ...
    const remaining = maxTotalTasks - items.length;
    const result = await listTasks(list.id, token, { ...options, showCompleted: false, maxTotalTasks: remaining });
    items.push(...result.items);
    totalFetched += result.totalFetched || result.items.length;
    truncated = truncated || Boolean(result.truncated);
  }

  return { items, lists, truncated, totalFetched };
}
...
export async function getTask(taskId: string, taskListId: string, explicitToken?: string): Promise<TaskItem> {
  ...
  if (!res.ok) throw new Error(await parseGoogleApiError(res, 'Failed to fetch task'));
  return normalizeTask(await res.json(), list);
}
...
export async function createTask(title: string, notes?: string, taskListId?: string, explicitToken?: string, parent?: string, previous?: string): Promise<TaskItem> {
  const token = await getGoogleFamilyAccessToken('tasks', explicitToken);
  const id = taskListId || (await getTaskLists(token)).items[0]?.id;
  if (!id) throw new Error('No Google Task lists found.');
  const params = new URLSearchParams();
  if (parent) params.set('parent', parent);
  if (previous) params.set('previous', previous);
  const suffix = params.toString() ? `?${params.toString()}` : '';
  const res = await fetch(`${TASKS_BASE_URL}/lists/${encodeURIComponent(id)}/tasks${suffix}`, {
    method: 'POST', headers: jsonHeaders(token), body: JSON.stringify({ title, ...(notes ? { notes } : {}) }),
  });
  if (!res.ok) throw new Error(await parseGoogleApiError(res, 'Failed to create task'));
  const lists = await getTaskLists(token);
  return normalizeTask(await res.json(), lists.items.find((item) => item.id === id));
}
...
export async function updateTask(taskId: string, taskListId: string, patch: Partial<Pick<TaskItem, 'title' | 'notes' | 'status' | 'due'>>, explicitToken?: string): Promise<TaskItem> {
  ...
  const res = await fetch(`${TASKS_BASE_URL}/lists/${encodeURIComponent(taskListId)}/tasks/${encodeURIComponent(taskId)}`, {
    method: 'PATCH', headers: jsonHeaders(token), body: JSON.stringify(body),
  });
  if (!res.ok) throw new Error(await parseGoogleApiError(res, 'Failed to update task'));
  const lists = await getTaskLists(token);
  return normalizeTask(await res.json(), lists.items.find((item) => item.id === taskListId));
}

export async function completeTask(taskId: string, taskListId: string, explicitToken?: string): Promise<TaskItem> {
  return updateTask(taskId, taskListId, { status: 'completed' }, explicitToken);
}

export async function deleteTask(taskId: string, taskListId: string, explicitToken?: string): Promise<{ id: string; deleted: true }> {
  const token = await getGoogleFamilyAccessToken('tasks', explicitToken);
  const res = await fetch(`${TASKS_BASE_URL}/lists/${encodeURIComponent(taskListId)}/tasks/${encodeURIComponent(taskId)}`, { method: 'DELETE', headers: authHeaders(token) });
  if (!res.ok) throw new Error(await parseGoogleApiError(res, 'Failed to delete task'));
...
export async function moveTask(taskId: string, taskListId: string, explicitToken?: string, options: { destinationTaskListId?: string; parent?: string; previous?: string } = {}): Promise<TaskItem> {
  const token = await getGoogleFamilyAccessToken('tasks', explicitToken);
  const params = new URLSearchParams();
  if (options.destinationTaskListId) params.set('destinationTasklist', options.destinationTaskListId);
  ...
  if (!res.ok) throw new Error(await parseGoogleApiError(res, 'Failed to move task'));
  const lists = await getTaskLists(token);
  const destinationId = options.destinationTaskListId || taskListId;
  return normalizeTask(await res.json(), lists.items.find((item) => item.id === destinationId));
}

/** Backward-compatible helper: all lists when no list ID is supplied, incomplete only by default. */
export async function getTasks(taskListId?: string, explicitToken?: string, options: ListTasksOptions = {}): Promise<ListTasksResult> {
  if (taskListId) return listTasks(taskListId, explicitToken, options);
  const result = await listAllPendingTasks(explicitToken, options);
  return { items: result.items, truncated: result.truncated, totalFetched: result.totalFetched };
}
