...
const ARTIFACT_TOOL_NAMES = new Set(['create_artifact', 'read_artifact', 'update_artifact', 'list_artifacts', 'rename_artifact']);
const GOOGLE_BACKED_WORKSPACE_WRITE_TOOLS = new Set(['create_google_doc', 'update_google_doc', 'link_google_doc', 'sync_to_google_doc', 'sync_from_google_doc']);
...
function withExternalActionConfirmation(tool: AgentToolDeclaration): AgentToolDeclaration {
  ...
  properties.userConfirmed = { type: 'BOOLEAN', description: 'Must be true only when the user has explicitly confirmed this external Google write/delete/revoke operation.' };
  const required = Array.from(new Set([...(parameters.required || []), 'userConfirmed']));
  return { ...tool, description: `${tool.description || ''} Explicit user confirmation is required before this operation changes Google data or authentication state.`, parameters: { ...parameters, properties, required } };
}
...
function withGoogleExposureMetadata(tool: AgentToolDeclaration, forceAuth = false): AgentToolDeclaration {
  ...
  if (actionClass === 'read') return { ...tool, capabilities: Array.from(new Set([...(tool.capabilities || []), 'google.read'])), effects: Array.from(new Set([...(tool.effects || []), 'read'])) };
  return { ...tool, capabilities: Array.from(new Set([...(tool.capabilities || []), 'google.write'])), effects: Array.from(new Set([...(tool.effects || []), actionClass === 'destructive' ? 'auth-change' : 'external-write'])) };
}
...
const googleAuthLifecyclePlugin: ToolPlugin = {
  ...
  authorize: googleAuthorization,
  execute: async ({ toolName, args, workspace }) => ({ result: await executeGoogleAuthLifecycleTool(toolName, args), updatedWorkspace: workspace }),
};

export const builtinToolPlugins: readonly ToolPlugin[] = [artifactToolPlugin, workspacePlugin, googleAgentPlugin, googleOperationalPlugin, googleAuthLifecyclePlugin];
